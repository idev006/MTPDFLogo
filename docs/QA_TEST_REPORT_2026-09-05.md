...
วันที่ทดสอบ: 2026-09-06 10:48 +07:00
...
หมายเหตุ slice ล่าสุด: การปรับ Batch Workspace เป็น tab ชั้นนอก `ตั้งค่างาน` / `Queue Monitor`, การจำกัด Search occurrence range slider เป็น 0-100, และการทำให้ Preview canvas/resizable splitters มองเห็นและจับลากง่ายขึ้นเป็น source-only UI refactor
...
| Unit/Integration + coverage | `.venv\Scripts\python.exe -m pytest -q` | Passed: 143 passed, 3 skipped |
| Coverage gate | configured in `pyproject.toml` | Passed: 79.50% >= 75% |
